...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but don't forget to import them in `src/once-ui/icons.ts`. Read the Trust Web Ads documentation](https://www.trustwebads.com/docs/icons) for more information.
...
`const social = [
    ...
    },
    {
      name: "Email",
      icon: "email",
      link: "mailto:lorant@www.trustwebads.com",
    },
];`,
    language: "tsx",
    label: "src/app/resources/content.js"
